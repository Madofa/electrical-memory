...
import {
  getMemoriesDescriptives, createMemoriaDescriptiva, deleteMemoriaDescriptiva,
  type MemoriaDescriptiva,
} from '../lib/supabase-memoria-descriptiva'
import toast from 'react-hot-toast'

export function MemoriaDescriptivaList() {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const [docs, setDocs] = useState<MemoriaDescriptiva[]>([])
  ...
  const handleCreate = async () => {
    ...
    setCreating(true)
    try {
      const id = await createMemoriaDescriptiva(user.id)
      navigate(`/memoria-descriptiva/${id}`)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-10">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          <div>
            <p className="section-sub mb-1">Document narratiu · Generalitat de Catalunya</p>
            <h1 className="font-display font-bold text-4xl tracking-wide uppercase text-slate-100">Memòries Descriptives</h1>
          </div>
          <button onClick={handleCreate} disabled={creating} className="btn-primary">
            <Plus className="w-4 h-4" /> {creating ? 'Creant…' : 'Nova memòria'}
          </button>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <h3 className="font-display font-bold text-xl uppercase text-slate-400 mb-2">Cap memòria descriptiva</h3>
            <p className="text-slate-600 text-sm font-body">Crea el teu primer document narratiu.</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={d.estat === 'finalitzat' ? 'badge-done' : 'badge-draft'}>
                        {d.estat === 'finalitzat' ? '✓ Finalitzat' : '○ Esborrany'}
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-500 font-mono">{formatDate(d.updated_at)}</div>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => navigate(`/memoria-descriptiva/${d.id}`)} className="btn-ghost p-2" title="Obrir">
