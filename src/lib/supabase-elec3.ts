...
export async function createElec3Doc(instaladorId: string): Promise<string> {
  const { data, error } = await supabase.from(TABLE)
    .insert({ instalador_id: instaladorId, nom: '', trams: [tramDerivacioIndividual()], estat: 'esborrany' })
    .select('id')
